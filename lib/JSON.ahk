...
/**
 * 避坑注意 ：
 * 0. 遇到数组时 for 循环会吞掉数组 只保留数组中的最后一个元素 因此要基于序列化后的原变量通过Loop加循环
 * 1. 默认的JSON.stringify会忽略undefined, 所以如果要序列化undefined, 需要手动指定replacer
 * 2. 默认的JSON.parse会忽略注释, 所以如果要序列化注释, 需要手动指定reviver
 */
